...
::            for PHP 4.3 C:\phpdev\php-4.3-Win32\cli\php.exe
  ...
  SET phpCli=C:\HRD\xampp\php\.\php.exe
...
IF '%1'=='' (
  ...
  "%phpCli%" -d include_path="C:\HRD\xampp\php\pear" "\xampp\php\phpdoc" %*
  GOTO :EOF
)
...
SET count=0
FOR /R "C:\HRD\xampp\php\pear\data\PhpDocumentor\user" %%I IN (*.ini) DO (
  SET /a count+=1
...
:run
SET found=1
ECHO Starting: "%phpCli%" "C:\HRD\xampp\php\phpdoc" -c "%iniFile%"
ECHO.
"%phpCli%"  -d include_path="C:\HRD\xampp\php\pear" "phpdoc" -c "%iniFile%"
GOTO :EOF
